...
const AcademicHome = () => {
  ...
  const fetchText = async () => {
    try {
      const response = await axios.get(
        "http://localhost:3663/api/academic/home"
      );
      const fetched = response.data.result[0];
      console.log("Fetched AcademicHome Text:", fetched.Content);
      setDeptText([fetched]); // wrap in array
      setContent(fetched.Content);
    } catch (err) {
      console.error("Error loading AcademicHome text:", err);
      setMessage("Error fetching AcademicHome text.");
    }
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await axios.put(`http://localhost:3663/api/academic/home/${id}`, {
        content: contentForStorage,
      });
      setMessage("AcademicHome text updated successfully!");
      setEditMode(false);
      fetchText();
    } catch (error) {
      console.error("Update error:", error);
      setMessage("Error updating AcademicHome text.");
    }
  };
  ...
  return (
    <div className="p-4">
      <h1 className="text-3xl font-inter font-semibold mb-10">Academic Home</h1>
      <style jsx>{`
      ...
      {deptText.length > 0 ? (
        ...
        <p>No Academic home text available.</p>
      )}
    </div>
  );
};
