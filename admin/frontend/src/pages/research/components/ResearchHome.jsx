...
const ResearchHome = () => {
  ...
  const fetchResearchText = async () => {
    try {
      ...
      setMessage(`Error fetching research text: ${error.response?.status} - ${errorMsg}`);
    }
  };
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      if (error.response?.status === 401) {
        ...
        setMessage(`Error uploading research text: ${error.response?.status} - ${errorMsg}`);
      }
    }
  };
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      if (error.response?.status === 401) {
        ...
        setMessage(`Error updating research text: ${error.response?.status} - ${errorMsg}`);
      }
    }
  };
        ...
        {researchText.length > 0 ? (
          <ul>
            {researchText.map((text) => (
              <li
                key={text.id}
                className={`mt-2 p-4 border rounded cursor-pointer ${selectedId === text.id ? "border-blue-500" : ""}`}
                onClick={() => {
