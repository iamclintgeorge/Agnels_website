...
const ResearchPdfManager = ({ section, topic }) => {
  ...
  const fetchPdfs = async () => {
    try {
      const response = await axios.get(`http://localhost:3663/api/department/research/pdf?section=${section}`, {
        withCredentials: true,
      });
      setCurrentPdfs(response.data);
      setMessage("");
      ...
      setMessage(`Error fetching PDFs: ${error.response?.status} - ${errorMsg}`);
    }
  };
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      await axios.post("http://localhost:3663/api/department/research/admin/pdf", formData, {
        headers: { "Content-Type": "multipart/form-data" },
        withCredentials: true,
      });
      setMessage("PDF(s) uploaded successfully!");
      ...
      console.error("Error uploading PDF:", error);
      const errorMsg = error.response?.data?.message || error.message;
      setMessage(`Error uploading PDF: ${error.response?.status} - ${errorMsg}`);
    }
  };
  ...
  const handleUpdate = async (id, file) => {
    ...
    try {
      // ✅ FIXED: Changed URL to the authenticated admin route
      await axios.put(`http://localhost:3663/api/department/research/admin/pdf/${id}`, formData, {
        headers: { "Content-Type": "multipart/form-data" },
        withCredentials: true,
      });
      setMessage("PDF updated successfully!");
  ...
  const handleDelete = async (id) => {
    try {
      // ✅ FIXED: Changed URL to the authenticated admin route
      await axios.delete(`http://localhost:3663/api/department/research/admin/pdf/${id}`, {
        withCredentials: true,
      });
      setMessage("PDF deleted successfully!");
      fetchPdfs();
    ...
    <div className="bg-white p-6 rounded-lg shadow-md">
      ...
      <div className="mb-6">
        ...
        {currentPdfs.length > 0 ? (
          <ul>
            {currentPdfs.map((pdf) => (
              <li key={pdf.id} className="flex items-center justify-between mb-2 p-2 border rounded">
                <a
                  href={`http://localhost:3663${pdf.content}`}
                  target="_blank"
                  rel="noopener noreferrer"
