...
const ElectricalHod = () => {
  ...
  // Check if user has permission to edit (electricalHod role or superAdmin)
  const canEdit = user && (user.role === "electricalHod" || user.role === "superAdmin");
  ...
  const fetchText = async () => {
    try {
      const response = await axios.get(
        "http://localhost:3663/api/hod-desk/electrical"
      );
      console.log("Fetched Electrical HOD Text:", response.data);
      ...
      console.error("Error loading Electrical HOD text:", err);
      setMessage("Error fetching Electrical HOD text.");
    }
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await axios.put(`http://localhost:3663/api/hod-desk/electrical/${id}`, {
        content: contentForStorage,
      });
      setMessage("Electrical HOD text updated successfully!");
      setEditMode(false);
      fetchText();
    } catch (error) {
      console.error("Update error:", error);
      setMessage("Error updating Electrical HOD text.");
    }
  };
  ...
  return (
    <div className="p-4">
      ...
      <h2 className="text-2xl font-bold mb-4">Electrical Engineering - HOD Desk</h2>
      ...
      {hodText.length > 0 ? (
        ...
        <div>
          <p>No Electrical HOD text available.</p>
          {canEdit && (
            <button
              onClick={() => setEditMode(true)}
