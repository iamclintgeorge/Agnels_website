...
const ItHod = () => {
  ...
  // Check if user has permission to edit (itHod role or superAdmin)
  const canEdit = user && (user.role === "itHod" || user.role === "superAdmin");
  ...
  const fetchText = async () => {
    try {
      ...
      console.error("Error loading IT HOD text:", err);
      setMessage("Error fetching IT HOD text.");
    }
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await axios.put(`http://localhost:3663/api/hod-desk/it/${id}`, {
        content: contentForStorage,
      });
      setMessage("IT HOD text updated successfully!");
      setEditMode(false);
      fetchText();
    } catch (error) {
      console.error("Update error:", error);
      setMessage("Error updating IT HOD text.");
    }
  };
