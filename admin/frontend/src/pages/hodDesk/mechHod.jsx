...
const MechHod = () => {
  ...
  // Check if user has permission to edit (mechHod role or superAdmin)
  const canEdit = user && (user.role === "mechHod" || user.role === "superAdmin");
  ...
  const fetchText = async () => {
    try {
      const response = await axios.get(
        "http://localhost:3663/api/hod-desk/mechanical"
      );
      console.log("Fetched Mechanical HOD Text:", response.data);
      ...
      console.error("Error loading Mechanical HOD text:", err);
      setMessage("Error fetching Mechanical HOD text.");
    }
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await axios.put(`http://localhost:3663/api/hod-desk/mechanical/${id}`, {
        content: contentForStorage,
      });
      setMessage("Mechanical HOD text updated successfully!");
      setEditMode(false);
      fetchText();
    } catch (error) {
      console.error("Update error:", error);
      setMessage("Error updating Mechanical HOD text.");
    }
  };
  ...
  return (
    <div className="p-4">
      ...
      <h2 className="text-2xl font-bold mb-4">Mechanical Engineering - HOD Desk</h2>
      ...
      {hodText.length > 0 ? (
        ...
        <div>
          <p>No Mechanical HOD text available.</p>
          {canEdit && (
            <button
              onClick={() => setEditMode(true)}
