...
const ExtcHod = () => {
  ...
  // Check if user has permission to edit (extcHod role or superAdmin)
  const canEdit = user && (user.role === "extcHod" || user.role === "superAdmin");
  ...
  const fetchText = async () => {
    try {
      const response = await axios.get(
        "http://localhost:3663/api/hod-desk/extc"
      );
      console.log("Fetched EXTC HOD Text:", response.data);
      ...
      console.error("Error loading EXTC HOD text:", err);
      setMessage("Error fetching EXTC HOD text.");
    }
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await axios.put(`http://localhost:3663/api/hod-desk/extc/${id}`, {
        content: contentForStorage,
      });
      setMessage("EXTC HOD text updated successfully!");
      setEditMode(false);
      fetchText();
    } catch (error) {
      console.error("Update error:", error);
      setMessage("Error updating EXTC HOD text.");
    }
  };
  ...
  return (
    <div className="p-4">
      ...
      <h2 className="text-2xl font-bold mb-4">EXTC - HOD Desk</h2>
      ...
      {hodText.length > 0 ? (
        ...
        <div>
          <p>No EXTC HOD text available.</p>
          {canEdit && (
            <button
              onClick={() => setEditMode(true)}
