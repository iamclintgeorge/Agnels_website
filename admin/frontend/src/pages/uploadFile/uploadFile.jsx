...
const UploadFile = () => {
  ...
  const fetchPdfs = async () => {
    try {
      const response = await axios.get(
        `http://localhost:3663/api/static-files/`
      );
  ...
  const handleUpload = async (e) => {
    ...
    try {
      await axios.post(
        "http://localhost:3663/api/static-files/create",
        formData,
        {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await axios.delete(
        `http://localhost:3663/api/static-files/${id}`,
  ...
  const handleCopyLink = async (url) => {
    try {
      await navigator.clipboard.writeText(
        `http://localhost:3663/cdn/static_pdfs/${url}`
      );
                ...
                >
                  <div className="flex flex-col">
                    ...
                    <p className="text-sm text-gray-500 truncate">
                      Link:{" "}
                      {`http://localhost:3663/cdn/static_pdfs/${pdf.filename}`}
                    </p>
                  </div>
