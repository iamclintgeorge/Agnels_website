...
const BSHHome = () => {
  ...
  const fetchText = async () => {
    try {
      const response = await axios.get(
        "http://localhost:3663/api/department/bsh/home"
      );
      console.log("Fetched BSH Department Text:", response.data);
      ...
      console.error("Error loading BSH department text:", err);
      setMessage("Error fetching BSH department text.");
    }
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await axios.put(`http://localhost:3663/api/department/bsh/home/${id}`, {
        content: contentForStorage,
      });
      setMessage("BSH department text updated successfully!");
      setEditMode(false);
      fetchText();
    } catch (error) {
      console.error("Update error:", error);
      setMessage("Error updating BSH department text.");
    }
  };
  ...
  return (
    <div className="p-4">
      ...
      <h2 className="text-2xl font-bold mb-4">Basic Science and Humanities - Home</h2>
      ...
      {deptText.length > 0 ? (
        ...
        <p>No BSH department home text available.</p>
      )}
    </div>
  );
};

export default BSHHome; 