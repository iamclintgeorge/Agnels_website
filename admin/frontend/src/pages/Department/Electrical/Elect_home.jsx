...
const ElectHome = () => {
  ...
  const fetchText = async () => {
    try {
      const response = await axios.get(
        "http://localhost:3663/api/department/electrical/home"
      );
      console.log("Fetched Electrical Department Text:", response.data);
      ...
      console.error("Error loading electrical department text:", err);
      setMessage("Error fetching electrical department text.");
    }
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await axios.put(`http://localhost:3663/api/department/electrical/home/${id}`, {
        content: contentForStorage,
      });
      setMessage("Electrical department text updated successfully!");
      setEditMode(false);
      fetchText();
    } catch (error) {
      console.error("Update error:", error);
      setMessage("Error updating electrical department text.");
    }
  };
  ...
  return (
    <div className="p-4">
      ...
      <h2 className="text-2xl font-bold mb-4">Electrical Engineering - Home</h2>
      ...
      {deptText.length > 0 ? (
        ...
        <p>No electrical department home text available.</p>
      )}
    </div>
  );
};

export default ElectHome; 