...
const optionalAnyUpload = (req, res, next) => {
  ...
  if (ct.includes("multipart/form-data")) {
    upload.any()(req, res, err =>
      err ? res.status(400).json({ success: false, message: err.message })
          : next()
    );
  } else next();
};
...
router.post ("/codeofconduct-create",  optionalAnyUpload, codeOfConductCreateController);
router.get  ("/codeofconduct",                         codeOfConductFetchController);
router.put  ("/codeofconduct/:id",  optionalAnyUpload, codeOfConductEditController);
...
router.post("/council-report-create",  optionalAnyUpload, councilReportCreateController);
router.get ("/council-reports",        councilReportsFetchController);
router.put ("/council-report/:id",     optionalAnyUpload, councilReportEditController);
router.put ("/delete-council-report/:id", councilReportDeleteController);

/* ========== 3. Professional Bodies ========== */
router.post("/professional-body-create",  optionalAnyUpload, professionalBodyCreateController);
router.get ("/professional-bodies",       professionalBodiesFetchController);
router.put ("/professional-body/:id",     optionalAnyUpload, professionalBodyEditController);
...
router.post("/nss-report-create",   optionalAnyUpload, nssActivityReportCreateController);
...
router.post("/cultural-activity-create", optionalAnyUpload, culturalActivityCreateController);
...
// Notices (PDF)
router.post("/anti-ragging-notice-create",  optionalAnyUpload, antiRaggingNoticeCreateController);
