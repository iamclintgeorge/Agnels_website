...
router.get('/overview', (req, res) => {
    const sql = 'SELECT * FROM training_placement_overview LIMIT 1';
    db.query(sql, (err, result) => {
        if (err) {
            console.error('Database error:', err);
            return res.status(500).json({ message: 'Failed to fetch overview', error: err.message });
        }
        res.json(result[0] || {});
    });
});
...
router.put('/overview', (req, res) => {
    ...
    const sql = `INSERT INTO training_placement_overview (description, vision, mission) 
                 VALUES (?, ?, ?)
    ...
    db.query(sql, [description, vision, mission], (err, result) => {
        if (err) {
            console.error('Database error:', err);
            return res.status(500).json({ message: 'Failed to update overview', error: err.message });
        }
...
router.post('/statistics', (req, res) => {
    ...
    db.query(sql, [academic_year, total_placements, average_package, highest_package, companies_visited], 
        (err, result) => {
            if (err) {
                console.error('Database error:', err);
                return res.status(500).json({ message: 'Failed to add statistics', error: err.message });
            }
...
router.delete('/statistics/:id', (req, res) => {
    ...
    db.query(sql, [req.params.id], (err, result) => {
        if (err) {
            console.error('Database error:', err);
            return res.status(500).json({ message: 'Failed to delete statistics', error: err.message });
        }
...
router.put('/statistics/:id', (req, res) => {
    ...
    db.query(sql, 
        [academic_year, total_placements, average_package, highest_package, companies_visited, req.params.id], 
        (err, result) => {
            if (err) {
                console.error('Database error:', err);
                return res.status(500).json({ message: 'Failed to update statistics', error: err.message });
            }
...
router.get('/recruiters', (req, res) => {
    ...
    db.query(sql, (err, result) => {
        if (err) {
            console.error('Database error:', err);
            return res.status(500).json({ message: 'Failed to fetch recruiters', error: err.message });
        }
        res.json(result);
    });
});
...
router.post('/recruiters', upload.single('logo'), (req, res) => {
    ...
    db.query(sql, [company_name, logo_path, description], (err, result) => {
        if (err) {
            console.error('Database error:', err);
            return res.status(500).json({ message: 'Failed to add recruiter', error: err.message });
        }
...
router.delete('/recruiters/:id', (req, res) => {
    ...
    db.query(selectSql, [req.params.id], (err, result) => {
        if (err) {
            console.error('Database error:', err);
            return res.status(500).json({ message: 'Failed to delete recruiter', error: err.message });
        }
        ...
        db.query(deleteSql, [req.params.id], (deleteErr, deleteResult) => {
            if (deleteErr) {
                ...
                return res.status(500).json({ message: 'Failed to delete recruiter', error: deleteErr.message });
            }
...
router.put('/recruiters/:id', upload.single('logo'), (req, res) => {
    ...
    db.query('SELECT logo_path FROM placement_recruiters WHERE id = ?', [req.params.id], (err, result) => {
        if (err) {
            console.error('Database error:', err);
            return res.status(500).json({ message: 'Failed to update recruiter', error: err.message });
        }
        ...
        db.query(sql, [updateFields, req.params.id], (updateErr, updateResult) => {
            if (updateErr) {
                console.error('Database error:', updateErr);
                return res.status(500).json({ message: 'Failed to update recruiter', error: updateErr.message });
            }
            res.json({ 
