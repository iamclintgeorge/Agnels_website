...
router.get('/resource/:resource', (req, res, next) => {
  ...
  if (departmentPermissions[userRole] && departmentPermissions[userRole].includes(resource)) {
    return next();
  }
  
  return res.status(403).json({ error: 'Access forbidden for this resource' });
}, getResourceActivities);
