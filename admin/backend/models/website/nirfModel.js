...
// --- ✨ NEW DELETE FUNCTION ---
/**
 * Deletes NIRF data for a specific year.
 * @param {string} year The year to delete data for.
 * @returns {Promise<object>}
 */
export const deleteNIRFData = async (year) => {
    const query = `DELETE FROM nirf_data WHERE year = ?`;
    try {
        const [result] = await db.promise().query(query, [year]);
        return result;
    } catch (error) {
        console.error("Database error deleting NIRF data:", error);
        throw error;
    }
};