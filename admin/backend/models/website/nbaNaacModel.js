...
//     const query = `
//       INSERT INTO nba_naac_home (id, content, image_urls, pdf_url, pdf_title)
//       VALUES (1, ?, ?, ?, ?)
...
//     try {
//       // Assuming home content is a single row with id = 1
//       const [rows] = await db
//         .promise()
//         .query("SELECT * FROM nba_naac_home WHERE id = 1");
...
//   /**
//    * Fetches a single file by its ID to get its URL before deletion.
...
//       const [rows] = await db
//         .promise()
//         .query("SELECT * FROM nba_naac_files WHERE id = ?", [id]);
//       return rows.length ? rows[0] : null;
//     } catch (error) {
//       console.error("Database error fetching file by ID:", error);
//       throw error;
//     }
//   },
...
//       const [result] = await db
//         .promise()
//         .query("DELETE FROM nba_naac_files WHERE id = ?", [id]);
//       return result;
//     } catch (error) {
...
export const NBANAACModel = {
  ...
  async saveFile(section, fileType, fileUrl, fileTitle, matrices = null) {
    const query = `
      INSERT INTO nba_naac_files (section, file_type, file_url, file_title, matrices)
      VALUES (?, ?, ?, ?, ?)
    `;
    try {
      const [result] = await db
        .promise()
        .query(query, [section, fileType, fileUrl, fileTitle, matrices]);
      return result;
    } catch (error) {
      console.error("Database error saving file:", error);
      throw error;
...
// --- The following models are from your original file and are not currently used by the NBA/NAAC page ---
