...
export const addStatistics = async (req, res) => {
  ...
  try {
    const result = await PlacementModel.addStatistics(
      academic_year,
      total_placements,
      average_package,
      highest_package,
...
export const updateStatistics = async (req, res) => {
  ...
  try {
    await PlacementModel.updateStatistics(
      id,
      academic_year,
      total_placements,
      average_package,
...
export const addRecruiter = async (req, res) => {
  ...
  const logo_path = req.file ? `/cdn/recruiters/${req.file.filename}` : null;
  try {
    const result = await PlacementModel.addRecruiter(
      company_name,
      logo_path,
      description
    );
...
export const deleteRecruiter = async (req, res) => {
  const { id } = req.params;
  try {
    const { logo_path, deleteResult } = await PlacementModel.deleteRecruiter(
      id
    );
    if (logo_path) {
...
export const updateRecruiter = async (req, res) => {
  ...
  const logo_path = req.file ? `/cdn/recruiters/${req.file.filename}` : null;
  try {
    await PlacementModel.updateRecruiter(
      id,
      company_name,
      description,
      logo_path
