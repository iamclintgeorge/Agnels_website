...
export const researchHomeController = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: `Error fetching research home text: ${error.message}` });
  }
};
...
export const researchPdfUpdateController = async (req, res) => {
  try {
    ...
    if (!oldPdf) {
      ...
      const tempPath = path.join(process.cwd(), "public", "Uploads", file.filename);
    ...
    if (result) {
      ...
      await fs.unlink(oldFilePath).catch(err => 
        console.error("Could not delete old file, it may not exist:", err.message)
      );
    }
...
export const researchPdfDeleteController = async (req, res) => {
  try {
    ...
    const filePath = path.join(process.cwd(), "public", pdf.content);
    await fs.unlink(filePath).catch((err) => console.log("Error deleting file (it may not exist):", err.message));
