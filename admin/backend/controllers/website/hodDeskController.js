...
export const getHodText = async (req, res) => {
  try {
    ...
    if (result.length === 0) {
      ...
      await createHodText(section, '<p>Welcome to the HOD Desk. Content will be added soon.</p>');
...
export const createHodTextEntry = async (req, res) => {
  try {
    ...
    if (existing.length > 0) {
      return res.status(400).json({ error: 'Content already exists for this section. Use update instead.' });
    }
