...
// To get the current directory of the file
...
export const getAllLogs = async (req, res) => {
  try {
    ...
    // Read the log files (combined.log and error.log)
    const combinedLogs = fs.readFileSync(logFilePath, "utf-8").split("\n");
    const errorLogs = fs.readFileSync(errorLogFilePath, "utf-8").split("\n");
    ...
    const logs = allLogs
      ...
      .map((log) => {
        try {
          ...
          // Format: "02-08-2025 22:09:28 [ERROR]: Server started {...}"
          const levelMatch = log.match(/\[(\w+)\]:/);
          const level = levelMatch ? levelMatch[1].toLowerCase() : "info";

          const jsonString = log.slice(log.indexOf("{")); // Slice to get the JSON part
          const parsedLog = JSON.parse(jsonString); // Parse JSON

          // Add the extracted level to the parsed log
          return {
            ...parsedLog,
            level: level,
          };
        } catch (e) {
          return null; // If JSON parsing fails, ignore the line
        }
      })
      .filter((log) => log !== null); // Filter out failed log parses
    ...
    if (username) {
      filteredLogs = filteredLogs.filter((log) =>
        log.created_by.toLowerCase().includes(username.toLowerCase())
      );
    }

    if (action) {
      filteredLogs = filteredLogs.filter((log) =>
        log.title.toLowerCase().includes(action.toLowerCase())
      );
    }

    if (resource) {
      filteredLogs = filteredLogs.filter((log) =>
        log.service.toLowerCase().includes(resource.toLowerCase())
      );
    }
    ...
    // Sort the logs by `created_on` field in descending order (latest logs first)
    filteredLogs.sort(
    ...
    // Return the filtered and paginated logs with field name mapping for frontend compatibility
    res.json({
      activities: paginatedLogs.map((log) => ({
        id: log.id,
        title: log.title,
        created_by: log.created_by,
        created_at: log.created_on, // Map created_on to created_at for frontend
        timestamp: log.created_on, // Also provide as timestamp
        description: log.description,
        service: log.service,
        source_ip: log.source_ip,
        ip_address: log.source_ip, // Map source_ip to ip_address for frontend
        level: log.level,
      })),
      pagination: {
