...
const generateDescription = (action, resource, resourceId, user) => {
  ...
  switch (action) {
    case 'CREATE':
      return `${userName} created new ${resourceName}${resourceId ? ` with ID ${resourceId}` : ''}`;
    case 'UPDATE':
      return `${userName} updated ${resourceName}${resourceId ? ` with ID ${resourceId}` : ''}`;
    case 'DELETE':
      return `${userName} deleted ${resourceName}${resourceId ? ` with ID ${resourceId}` : ''}`;
    case 'READ':
      return `${userName} accessed ${resourceName}${resourceId ? ` with ID ${resourceId}` : ''}`;
    default:
  ...
  res.on('finish', async () => {
    try {
      ...
      const activityData = {
        ...
        description: generateDescription(action, resource, resourceId, req.user),
      ...
      if (action === 'CREATE' && responseData && typeof responseData === 'object') {
