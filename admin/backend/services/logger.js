...
const logger = winston.createLogger({
  ...
  format: customFormat,
  transports: [
    // File transport for general logs (without color)
    new winston.transports.File({
      filename: path.join(logsDir, "error.log"),
      level: "error",
      maxsize: 5242880, // 5MB
      maxFiles: 5,
      format: customFormat, // Use custom format for file logs (no color)
    }),
    new winston.transports.File({
      filename: path.join(logsDir, "combined.log"),
      maxsize: 5242880, // 5MB
    ...
    // Console transport for development (with color)
    new winston.transports.Console({
      level: "debug",
      format: winston.format.combine(winston.format.colorize(), customFormat), // Colorized for console
    }),
  ],
});
