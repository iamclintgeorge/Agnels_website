  ...
  useEffect(() => {
    const fetchNews = async () => {
      try {
        ...
        const titles = res.data?.result?.map((row) => row) || []; // Store the full row to access attachment as well
        setNewsItems(titles);
      } catch (err) {
        console.error("Error fetching news:", err);
        // Optional fallback (leave empty array if you prefer)
        setNewsItems([{ subject: "Unable to load news at the moment" }]);
      }
    };
          ...
          {newsItems.map((item, idx) => (
            <span key={idx} className="mr-4 flex items-center">
              ...
              {item.attachment ? (
                ...
                <span dangerouslySetInnerHTML={{ __html: item.subject }}></span>
              )}
