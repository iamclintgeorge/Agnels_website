...
const FileDisplayComponent = ({ sectionName, title }) => {
  ...
  useEffect(() => {
    const fetchFiles = async () => {
      try {
        const response = await axios.get(`${backendBaseUrl}/api/nba-naac/files/${sectionName}`);
        setFiles(response.data);
      } catch (error) {
        console.error(`Error fetching ${sectionName} files:`, error);
      } finally {
      ...
      {files.length > 0 ? (
        <div className="space-y-4">
          {files.map((file) => (
            <div key={file.id} className="border p-4 rounded bg-white shadow-sm">
              <h3 className="text-xl font-medium mb-2">{file.file_title}</h3>
              <a href={`${backendBaseUrl}${file.file_url}`} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">
                View/Download
              </a>
            </div>
          ))}
...
const NAACAppealsContent = () => {
    ...
    useEffect(() => {
        const fetchFiles = async () => {
            try {
                const response = await axios.get(`${backendBaseUrl}/api/nba-naac/files/NAAC Appeals`);
                setFiles(response.data);
            } catch (error) {
        ...
        <div className="p-8 bg-gray-50 min-h-screen font-sans">
            <div className="border p-6 bg-white shadow-md">
                <div className="text-center border-b pb-4">
                    ...
                    <p className="text-gray-600 mt-1">{"{Click on the Link below to View/Download}"}</p>
                </div>
                <div className="mt-4">
                    <table className="w-full border-collapse">
                        <thead>
                            <tr className="border-b">
                                <th className="p-3 text-left font-semibold text-gray-700 w-1/4">Matrices</th>
                                <th className="p-3 text-left font-semibold text-gray-700">Supporting information</th>
                            </tr>
                        </thead>
                        <tbody>
                            {files.length > 0 ? files.map((file) => (
                                ...
                                <tr>
                                    <td colSpan="2" className="p-3 text-center text-gray-500">No appeal documents found.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
