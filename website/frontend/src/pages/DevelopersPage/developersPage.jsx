  ...
  const DeveloperCard = ({
    ...
    return (
      <div className="flex flex-col items-center">
        ...
        <h4 className={`text-center font-medium ${textClasses[size]}`}>
          {name}
        </h4>
        {department && (
          <p className="text-xs text-gray-600 text-center mt-1">{department}</p>
        )}
            ...
            <div className="ml-[240px] flex flex-col items-center">
              ...
              <div className="max-w-6xl mx-auto">
                {firstSectionRows.map((row, rowIndex) => (
                  <div
                    key={`row-${rowIndex}`}
                    className="grid grid-cols-5 gap-4 mb-8 md:mb-12"
                  >
                    {row.map((dev, devIndex) => {
                      // Extract department from position (which contains year info)
                      const departmentMatch = dev.position
                        ? dev.position.match(/(Department of [^(]+)/)
                        : null;
